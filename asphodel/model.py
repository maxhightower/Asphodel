...
class Simulation:
    ...
    def __init__(self, config: ScenarioConfig):
        ...
        # heterogeneous for the commute topology, or an explicit per-zone vector
        # (e.g. real OSM building density via GraphParams.population).
    ...
    def step(self, frozen_internal=None) -> TickRecord:
        ...
        if infra.enabled:
            ...
            # Player resource allocation (staffing_support) props up populated zones.
            self.staffing = np.where(
                self.N0 > 0,
                np.clip(available / self.N0_safe + self.staffing_support, 0.0, 1.0),
                1.0,
            )
