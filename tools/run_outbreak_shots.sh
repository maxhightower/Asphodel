#!/usr/bin/env bash
# ASPHODEL_OUTBREAK_V1 visual evidence: the real IsometricWorld scene
...
PYTHONPATH=. python3 -m asphodel.bridge.server --host 127.0.0.1 --port "$PORT" \
    > /tmp/asph_livecert/outbreak_shots_server.out 2>&1 &
SPID=$!
...
xvfb-run -a -s "-screen 0 1280x720x24" env LIBGL_ALWAYS_SOFTWARE=1 "$GODOT" --path godot \
    ...
    res://tests/OutbreakShot.tscn -- --bundle "$BUNDLE" --citizen "$CITIZEN" --dir "$(pwd)/$OUT"
CODE=$?
